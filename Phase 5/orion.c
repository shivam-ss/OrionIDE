#include "color.h"
#include "strfunc.h"
#include "filefunc.h"
#include "syntaxerrors.h"
#include "spellcheck.h"
#include "cursor.h"

#define NO_OF_BRACES 100
#define NO_OF_PARANTHESIS 100
#define CODE_X 7
#define CODE_Y 16
#define TAB_SIZE 4

int countOpenBraces = 0;
int Braces[NO_OF_BRACES][2];//stores location of open braces with line no. and col no.
int countOpenParens = 0;
int Parens[NO_OF_PARANTHESIS][2];//stores location of open paranthesis with line no. and col no.
WORD DEFAULT_COLOR;//Default color of text in console
int NOL;//Number of lines
char** str;//Code
int CURSOR_X;
int CURSOR_Y;
char compiler[8];
bool changedWithoutSave;

void printName(bool printShortcuts);
void printCoordinates();
void selectCompiler(const char* filename);
bool compile(const char* filename);
void execute();
void checkSpelling(int LINE_NUMBER);
void doAutofill(bool showHint, const char* filename);
void insertCharacter(char *c);
void insert(char *c, const char* filename);
void error();
void replaceTabWithSpaces();
void printFilename(const char* filename);
void oldFileEdit(const char* filename);
void newFileEdit(const char *filename);


void printName(bool printShortcuts){
	system("cls");
	changeTextColor(FOREGROUND_BLUE);
	printf("\t     ****     *****      *******     ****     **     *   \n");
	printf("\t    *    *    *    *        *       *    *    * *    *   \n");
	printf("\t   *      *   *****         *      *      *   *  *   *   \n");
	printf("\t   *      *   *    *        *      *      *   *   *  *   \n");
	printf("\t    *    *    *     *       *       *    *    *    * *   \n");
	printf("\t     ****     *      *   *******     ****     *     **   \n\n\n");
	
	if(printShortcuts){
		changeTextColor(FOREGROUND_INTENSITY);
		printf("SHORTCUTS:\n");
		printf("Ctrl + s -> Save\n");
		printf("Ctrl + c -> Compile and Run\n");
		printf("Ctrl + o -> Autofill\n");
		printf("Esc      -> Exit\n\n\n");
	}
	changeTextColor(DEFAULT_COLOR);
}
void printCoordinates(){
	int X = getCursorX();
	int Y = getCursorY();
	setCursor(0,CODE_Y-2);
	printf("\r                                           ");
	printf("\rLine: %d\tCol: %d", Y-CODE_Y, X-CODE_X);
	setCursor(X,Y);
}
void selectCompiler(const char* filename){
	char* ext = strchr(filename, '.');
	if(strcmp(ext,".c")==0)
		strcpy(compiler,"gcc");
	else if(strcmp(ext,".cpp")==0)
		strcpy(compiler,"g++");
	else if(strcmp(ext,".py")==0)
		strcpy(compiler,"python");
	else if(strcmp(ext,".java")==0)
		strcpy(compiler,"javac");
}
bool compile(const char* filename){
	char command[50];
	strcpy(command, compiler);
	strcat(command," ");
	strcat(command,filename);
	strcat(command," -o temp.exe");
    system(command);
    if(!isFileExists("temp.exe"))
    	return false;
    return true;
}
void execute(){
	system("start cmd /k opencmd.bat");
}
void checkSpelling(int LINE_NUMBER){
	int X = getCursorX();
	int Y = getCursorY();
//	setCursor(CODE_X+1,CODE_Y+LINE_NUMBER);
//	
//	int i = LINE_NUMBER-1;
//	
//	char temp[LINE_LENGTH];
//	strcpy(temp, str[i]);
//	int j;
//	char* token;
//	token = strtok(temp," ");
//	printf("\r%d\t",i+1);
//	
//	while (token != NULL) {
//		char word[30];
//		strcpy(word, token);
//		if(checkkeyword(word)){
//			changeTextColor(FOREGROUND_GREEN);
//			printf("%s%s",space,word);
//			changeTextColor(DEFAULT_COLOR);
//		}
//		else if(!checkword(word)){
//			changeTextColor(FOREGROUND_RED);
//			printf("%s%s",space,word);
//			changeTextColor(DEFAULT_COLOR);
//		}
//		else
//			printf("%s%s",word,space);
//		for(j=0; j<10; j++)
//			strcat(space," ");
//		for(j=0; j<10; j++){
//			space[strlen(space)-1] = '\0';
//			token = strtok(temp,space);
//			token = strtok(NULL, space);
//			if(token != NULL)
//				break;
//		}
//	}
	setCursor(X,Y);
}
void insertCharacter(char *c){
	int X = getCursorX();
	int Y = getCursorY();
	
	int COL = X - CODE_X;
	int LINE = Y - CODE_Y;
	substrInsert(str[LINE-1],c,COL-1);
	printf("\r%d\t%s",LINE,str[LINE-1]);
	printf("                   ");
	setCursor(X+1,Y);
	checkSpelling(LINE);
}
void insert(char* c, const char* filename){
	changedWithoutSave = true;
	printFilename(filename);
	insertCharacter(c);
	if(c[0] == '{')
		insertCharacter("}");
	else if(c[0] == '(')
		insertCharacter(")");
	else if(c[0] == '[')
		insertCharacter("]");
	else if(c[0] == '"')
		insertCharacter(c);
	else if(c[0] == '\'')
		insertCharacter(c);
	else{
		int X = getCursorX();
		int Y = getCursorY();
		
		int COL = X - CODE_X;
		int LINE = Y - CODE_Y;
		if(COL-1 == strlen(str[LINE-1]))
			doAutofill(true,filename);
		return;
	}
	setCursor(getCursorX()-1,getCursorY());
}
void doAutofill(bool showHint, const char* filename){
	int X = getCursorX();
	int Y = getCursorY();
	
	int COL = X - CODE_X;
	int LINE = Y - CODE_Y;
	
	char temp[LINE_LENGTH];
	strcpy(temp,str[LINE-1]);
	
	if(COL == 1 || temp[COL-2] == ' ')
		return;
	int j =  COL-2;
	while(j >=0){
		if(temp[j] == ' ')
			break;
		j--;
	}
	
	int start_index = j+1;
	int end_index = COL-1;
	char *word = substr(temp,start_index,end_index);
	
	int prevlen = strlen(word);
	autocomplete(word);
	word  = substr(word,prevlen,strlen(word));
	if(showHint){
		changeTextColor(FOREGROUND_INTENSITY);
		printf("%s",word);
		changeTextColor(DEFAULT_COLOR);
		setCursor(X,Y);
	}
	else{
		char c[2];
		c[1] = '\0';
		for(j=0; j<strlen(word); j++){
			c[0] = word[j];
			insert(c, filename);
		}
	}
}
void error(){
	changeTextColor(FOREGROUND_BLUE);
	printf("\nPress Enter to go back to Orion...");
	changeTextColor(DEFAULT_COLOR);
	while(getch() != 13);
}
void replaceTabWithSpaces(){
	int i;
	for(i=0; i<NOL; i++){
		int index = strpos(str[i],"\t",0);
		if(index == -1)
			continue;
		substrDelete(str[i],index,index+1);
		int j;
		for(j=0; j<TAB_SIZE; j++)
			substrInsert(str[i]," ",index);
	}
}
void printFilename(const char* filename){
	int X = getCursorX();
	int Y = getCursorY();
	setCursor(0,CODE_Y);
	printf("\r%s ",filename);
	if(changedWithoutSave)
		printf("\r%s*",filename);
	setCursor(X,Y);
}
void oldFileEdit(const char* filename){
	static bool CURSOR = false;
	printName(true);

	printf("\n%s\n",filename);
	if(NOL == -1){
		changedWithoutSave = false;
		free(str);
		str = readArrayOfStrings(filename);
		replaceTabWithSpaces();
		CURSOR = false;
	}
		
	int i;
	for(i=0; i<NOL; i++){
		printf("%d\t%s",i+1,str[i]);
		if(i != NOL-1)
			printf("\n");
		checkSpelling(i+1);
	}
	
	char c[2];
	c[1] = '\0';
	printCoordinates();
	if(CURSOR)
		setCursor(CURSOR_X,CURSOR_Y);
	bool compileAndRun = false;
	
	while(1){
		c[0] = getch();
		
		if(c[0]== 13){	//13 ASCII value of ENTER
			int X = getCursorX();
			int Y = getCursorY();
			
			int COL = X - CODE_X;
			int LINE = Y - CODE_Y;
			
			for(i=NOL; i>LINE; i--)
				strcpy(str[i],str[i-1]);
			char *add = substr(str[LINE-1], COL-1, strlen(str[LINE-1]));
			strcpy(str[LINE], add);
			str[LINE-1][COL-1] = '\0';
			NOL++;
			CURSOR_X = CODE_X+1;
			CURSOR_Y = CODE_Y+LINE+1;
			CURSOR = true;
			changedWithoutSave = true;
			printFilename(filename);
			oldFileEdit(filename);
			return;
		}
		else if(c[0] == 8){	//8 ASCII value of BACKSPACE
			int X = getCursorX();
			int Y = getCursorY();
			
			int COL = X - CODE_X;
			int LINE = Y - CODE_Y;
			if(COL==1 && LINE ==1)
				continue;
			if(COL==1){
				int len = strlen(str[LINE-2]);
				strcat(str[LINE-2],str[LINE-1]);
				for(i=LINE; i<NOL; i++)
					strcpy(str[i-1],str[i]);
				NOL--;
				strcpy(str[NOL],"");
				CURSOR_X = CODE_X+len+1;
				CURSOR_Y = CODE_Y+LINE-1;
				CURSOR = true;
				changedWithoutSave = true;
				printFilename(filename);
				oldFileEdit(filename);
				return;
			}
			changedWithoutSave = true;
			printFilename(filename);
			substrDelete(str[LINE-1],COL-2,COL-1);
			printf("\r%d\t%s ",LINE,str[LINE-1]);
			printf("                      ");
			setCursor(X-1,Y);
			checkSpelling(LINE);
		}
		else if(c[0] == 15){	//15 ASCII value of CTRL + o
			doAutofill(false, filename);
		}
		else if(c[0] == 19 ){	//15 ASCII value of CTRL + s
			if(changedWithoutSave == false)
				continue;
			writeArrayOfStrings(str, NOL, filename);
			changedWithoutSave = false;
			printFilename(filename);
		}
		else if(32<=c[0] && c[0]<=126){
			insert(c,filename);
		}
		else if(c[0] == 9){	//9 ASCII value of TAB
			int j;
			for(j=0; j<TAB_SIZE; j++)
				insertCharacter(" ");
			changedWithoutSave = true;
			printFilename(filename);
		}
		else if(c[0] == 3){	//3 ASCII value of CTRL + c
			if(changedWithoutSave == false)
				writeArrayOfStrings(str, NOL, filename);
			changedWithoutSave = false;
			printFilename(filename);
			CURSOR_X = getCursorX();
			CURSOR_Y = getCursorY();
			CURSOR = true;
			compileAndRun = true;
			break;
		}
		else if(c[0] == -32){	//Arrow Key is Pressed
			//K - left arrow
			//H - up arrow
			//P - down arrow
			//M - right arrow
			c[0] = getch();
			int X = getCursorX();
			int Y = getCursorY();
			
			int COL = X - CODE_X;
			int LINE = Y - CODE_Y;
			
			if(c[0] == 'K'){
				if(X != CODE_X+1)
					setCursor(X-1, Y);
				else if(X == CODE_X+1 && LINE != 1)
					setCursor(strlen(str[LINE-2])+CODE_X+1, Y-1);
			}
			else if(c[0] == 'H' && Y != CODE_Y+1)
				setCursor(X, Y-1);
			else if(c[0] == 'P' && Y != CODE_Y+NOL)
				setCursor(X, Y+1);
			else if(c[0] == 'M'){
				if(COL-1 != strlen(str[LINE-1]))
					setCursor(X+1, Y);
				else if(COL-1 == strlen(str[LINE-1]) && LINE< NOL)
					setCursor(CODE_X+1,Y+1);
			}
			X = getCursorX();
			Y = getCursorY();
			
			COL = X - CODE_X;
			LINE = Y - CODE_Y;
			
			if((COL-1) >= strlen(str[LINE-1]))	//So that cursor doesn't go beyond line's length
				setCursor(strlen(str[LINE-1])+CODE_X+1,Y);
		}
		else if(c[0]==27){	//27 ASCII value of ESC
			setCursor(0,CODE_Y+NOL);
			system("cls");
			exit(0);
		}
	
		fflush(stdin);
		printCoordinates();
	}
	if(compile){
		setCursor(0, CODE_Y+NOL+1);
		changeTextColor(FOREGROUND_RED);
		fflush(stdin);
		countOpenBraces = 0;
		countOpenParens = 0;
		bool flag = false;
		for(i=1;i<=NOL;i++){
			if(checkSyntax(str,i))
				flag = true;
		}
		if(flag)
			error();
		else if(countOpenBraces > 0 ){
			int LINE = Braces[countOpenBraces-1][0];
			int COL = Braces[countOpenBraces-1][1]+1;
			printf("\nOpen braces '{' at line %d col %d", LINE, COL);
			error();
		}
		else if(countOpenParens > 0 ){
			int LINE = Parens[countOpenParens-1][0];
			int COL = Parens[countOpenParens-1][1]+1;
			printf("\nOpen braces '(' at line %d col %d", LINE, COL);
			error();
		}	
		else{
			if(compile(filename))
				execute();
			else
				error();
		}
		changeTextColor(DEFAULT_COLOR);
	}
	oldFileEdit(filename);
}

void newFileEdit(const char *filename){
	writeString(" ",filename);
	oldFileEdit(filename);
}

int main(int argc,char* argv[]){
	char filename[50];
	saveDefaultAttributes();
	if(argc==1){
		printName(false);
	    printf("Enter filename(with extension):\t");
	    scanf("%s",filename);
	}
	else
		strcpy(filename, argv[1]);
	printName(true);
	
	selectCompiler(filename);
	
	NOL = -1;
    if(isFileExists(filename))
    	oldFileEdit(filename);
    else
    	newFileEdit(filename);
	   
	return 0;
}
